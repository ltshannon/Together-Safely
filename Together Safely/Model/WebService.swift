...
*/
    ...
    func acceptInviteToGroup(groupId: String, completion: @escaping (Bool) -> Void)  {
        ...
        Auth.auth().signIn(with: credential) { (res, error) in
            ...
            currentUser?.getIDTokenForcingRefresh(true) { idToken, error in
                ...
                let idToken =  UserDefaults.standard.value(forKey: "idToken") as? String ?? ""
            ...
            currentUser?.getIDTokenForcingRefresh(true) { idToken, error in
                ...
                let task = session.dataTask(with: request) { (data, response, error) in
                    if let error = error {
                        ...
                        let result = try? JSONDecoder().decode(CreateInviteResponse.self, from: data)
                        if let result = result {
                            if result.message.contains("User added to group.") {
                                completion(true)
                                return
                            }
    ...
    func checkPhoneNumbers(phoneNumbers: [String], completion: @escaping ([String], Error?) -> Void)  {
        
        let url = URL(string: "https://us-central1-together-c537f.cloudfunctions.net/api/invitablePhoneNumbers")!
        var request = URLRequest(url: url)
                    ...
                    if let error = error {
                        ...
                        if let result = result {
                            if result.invitablePhoneNumbers.count == 0 {
                                let error: Error = NSLocalizedString("No error", comment: "My error") as! Error
                                completion([], error)
                                return
                            }
                            let error: Error = NSLocalizedString("Called failed", comment: "My error") as! Error
                            completion(result.invitablePhoneNumbers, error)
                        }
                    } else {
    ...
    func createNewGroup(name: String, successful: @escaping (Bool) -> Void)  {
